...
class BuildingTreehouse(renderer: ObjectRenderer) : BuildingBase(renderer){

    constructor() : this(ObjectRenderer("buildings", Vector2(1.0,1.0), 0)){
        renderer.animator!!.addAnimation("build", AnimationLibrary.buildingTreehouse)
        treehouseCount++
    }

    companion object{
        var treehouseCount = 0;
    }
    ...
    override fun finishBuilding() {
        if(!destroyed) HeroesBuilder.placeHero(HeroesEnum.BOWMAN, getWorldPosition())
